...
class ShiritoriSolver
  ...
  def initialize(words)
    ...
    nodes_begin = @nodes + ["^"] # 起点となれる文字（現れる全文字＋ダミー起点文字）
    nodes_end = @nodes + ["$"]   # 終点となれる文字（現れる全文字＋ダミー終点文字）
    ...
    # 初期状態では制約式の数は(@nodes.size)+2。あとで増えていく
    rows = problem.add_rows(@nodes.size + 2)
    @nodes.each_with_index do |n, i|
      rows[i].name = "C_#{n}" # ダミー以外の文字については入次数＝出次数
      rows[i].set_bounds(Rglpk::GLP_FX, 0.0, 0.0)
    end
    rows[@nodes.size].name = "C_^" # ダミー起点文字からの出次数は1
    rows[@nodes.size].set_bounds(Rglpk::GLP_FX, 1.0, 1.0)
    rows[@nodes.size+1].name = "C_$" # ダミー終点文字への入次数は1
    rows[@nodes.size+1].set_bounds(Rglpk::GLP_FX, 1.0, 1.0)
    ...
    nodes_begin.each do |nb|
      nodes_end.each do |ne|
        ...
        if nb == '^' || ne == '$'
          ...
          if @links["#{nb}#{ne}"].empty?
            cols[i].set_bounds(Rglpk::GLP_FX, 0.0, 0.0)
          else
            cols[i].set_bounds(Rglpk::GLP_DB, 0.0, @links["#{nb}#{ne}"].size.to_f)
          end
        end
    ...
    nodes_begin.each_with_index do |nb, i|
      nodes_end.each_with_index do |ne, j|
        mat << (i == @nodes.size ? 1 : 0)
      end
    end
    # (3. ダミー終点文字への入次数は1)
    nodes_begin.each_with_index do |nb, i|
      nodes_end.each_with_index do |ne, j|
        mat << (j == @nodes.size ? 1 : 0)
      end
    end
...
STDERR.puts "Current upper bound of length: #{len}"
      # 結果として得られた経路が連結か？
      connected_edges, connected_nodes = retrieve_connected_edges_and_nodes(@nodes, nodes_begin, nodes_end, cols)
        
      # ----- 使った辺がすべて連結なら、そこで終了
      if len == connected_edges.size
        best_edges = connected_edges if connected_edges.size > best_edges.size
        break
      end
        
      # ----- 連結でない場合、
      if len < best_edges.size
      ...
      # 今回連結と判断された辺が暫定1位か判定
      best_edges = connected_edges if connected_edges.size > best_edges.size
STDERR.puts "Current best length: #{best_edges.size}"
...
STDERR.puts "k == #{k}"
    ...
    # ---------- 具体的に最長経路を出力
    best_nodes = best_edges.each_item.map{ |e| e.chars.to_a }.flatten.uniq
  ...
  attr_reader :cycles, :noncycle_path, :path, :links, :nodes, :words
    ...
    results.each do |res|
      ...
      res.each_with_index do |r, i|
        if result_count.count(r) == 1
          if @links[r].size == 1
            paths.last << @links[r][0]
          else
            paths.last << {:option => @links[r], :dup => 1}
          end
        elsif result_count.count(r) == @links[r].size
          paths.last << @links[r][used_count[r]]
          used_count[r] += 1
        else
          paths.last << {:option => @links[r], :dup => result_count.count(r), :key => r, :note => note_list[r]}
        end
      end
    end
    
    note_list.keys.sort_by{ |r| note_list[r] }.each do |r|
      notes << {:begin => r[0], :end => r[1], :words => @links[r], :count => result_count.count(r)}
    end
    
    Result.new(paths, notes)
  end

  def extract_result(result = @path, out = STDOUT)
